...
static void _ticker_layout_hide_cb(void *data, Evas_Object *obj, const char *emission, const char *source)
{
}

static void _ticker_layout_mouse_down_cb(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
	_ticker_layout_hide_cb(data, NULL, NULL, NULL);
}
...
static Evas_Object *_ticker_layout_create(Evas_Object *win)
{
	...
	//elm_object_signal_callback_add(layout, "request,hide", "", _ticker_layout_hide_cb, NULL);
...
static Evas_Object *_ticker_window_create(struct appdata *ad)
{
	...
	//evas_object_smart_callback_add(win, "wm,rotation,changed", NULL, ad);
	//evas_object_smart_callback_add(win, "sub-object-del", NULL, NULL);

	//_rotate_cb(ad, ticker_win, NULL);
...
static void _ticker_noti_destroy(ticker_info_s *ticker_info)
{
	...
	if (!ticker.current && !ticker.pending)
	       _ticker_view_destroy();
